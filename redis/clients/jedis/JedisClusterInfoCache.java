...
public class JedisClusterInfoCache
{
    ...
    public synchronized void assignSlotsToNode(final List<Integer> targetSlots, final HostAndPort targetNode) {
        this.w.lock();
        try {
            JedisPool targetPool = this.nodes.get(getNodeKey(targetNode));
            if (targetPool == null) {
    ...
    public synchronized JedisPool getNode(final String nodeKey) {
        this.r.lock();
        try {
    ...
    public synchronized JedisPool getSlotPool(final int slot) {
        this.r.lock();
        try {
