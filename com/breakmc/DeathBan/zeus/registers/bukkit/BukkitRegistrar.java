package com.breakmc.DeathBan.zeus.registers.bukkit;

import com.breakmc.DeathBan.zeus.registers.*;
import com.breakmc.DeathBan.zeus.*;
import java.lang.annotation.*;
import java.util.*;
import com.breakmc.DeathBan.zeus.exceptions.*;
import org.bukkit.*;
        ...
        for (final Method m : obj.getClass().getMethods()) {
            if (m.isAnnotationPresent(Command.class)) {
                ...
                if (command.name().equalsIgnoreCase(name)) {
                    try {
                        final Constructor<?> commandConstructor = BukkitZeusCommand.class.getDeclaredConstructor(String.class, String.class, String.class, List.class, Object.class);
                        commandConstructor.setAccessible(true);
                        System.out.println("Successfully hooked into org.bukkit.command.Command");
                        final BukkitZeusCommand command2 = (BukkitZeusCommand)commandConstructor.newInstance(command.name(), command.desc(), command.usage(), Arrays.asList(command.aliases()), obj);
        ...
        for (final Method m : obj.getClass().getMethods()) {
            if (m.isAnnotationPresent(SubCommand.class)) {
                ...
                try {
                    ...
                    final BukkitZeusSubCommand subCommand = new BukkitZeusSubCommand(sc.parent(), sc.name(), sc.aliases(), sc.permission(), obj);
